...
const CreateProduct = () => {
        ...
        if (url) {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    //comment these lines to not automatically logout the person
                    //localStorage.setItem('jwt', data.token)
                    //localStorage.setItem('user', JSON.stringify(data.user))
                    M.toast({ html: 'Successfully created!', classes: "#43a047 green darken-1" })
                    navigate('/')
                }
            })
            .catch(err => {
    ...
    return (
        <div className="card input-field" style={{
            ...
            <input type="number" placeholder="Price" min="0" step="0.1" value={price} onChange={(e) => setPrice(e.target.value)}/>
            <input type="text" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)}/>
            <input list="products" placeholder="Category" value={category} onChange={(e) => setCategory(e.target.value)}/>
